...
flags.DEFINE_integer('vocabulary_size', 506634,'vocabulary_size (494796)')
flags.DEFINE_integer('emdedding_dim', 200,'embedding dim (128)')
flags.DEFINE_integer('hidden_neural_size', 200,'LSTM hidden neural size (128)')
flags.DEFINE_integer('hidden_layer_num', 1,'LSTM hidden layer num')
flags.DEFINE_string('dataset_path', '../../BDCI2017-360/BDCI2017-360-Semi/train_seg_clean.pkl','dataset path (data/subj0.pkl)')
flags.DEFINE_integer('max_len', 300, 'max_len of training sentence (40)')
flags.DEFINE_integer('valid_num', 10, 'epoch num of validation')
...
flags.DEFINE_integer('num_epoch', 10, 'num epoch')  # 60
flags.DEFINE_integer('max_decay_epoch', 10, 'num epoch')  # 30
flags.DEFINE_integer('max_grad_norm', 5, 'max_grad_norm')  # 5
...
flags.DEFINE_bool('using_mydata', True, 'if use mydata')
...
class Config(object):
    ...
    using_mydata = FLAGS.using_mydata
...
if __name__ == "__main__":
    ...
    tf.app.run() # tf.app.run(function) ,不指明函数则是main
